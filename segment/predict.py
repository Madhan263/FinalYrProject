...
my_font1=('Times New Roman', 16, 'bold')

#bg = PhotoImage(file='BLUE.png')
...
l1 = tk.Label(root,text='Upload Files & display',width=40,font=my_font1,bg='#D35400',
                   fg='white',)  
l1.place(x=370, y=220, width=300)
b1 = tk.Button(root, text='Select Video', 
   width=20, command = lambda:looping(), activebackground='#000080', bg='#D35400')
b1.place(x=350, y=370, width=150, height=30)
entry = tk.Entry(root)
def on_entry_click(event):
    if entry.get() == 'Enter your text here':
        entry.delete(0, tk.END) # Remove the placeholder text
entry.configure(background='#D3D3D3')
entry.insert(0, 'No file Choosen')
entry.bind('<FocusIn>', on_entry_click)
entry.place(x=500,y=370,width=200, height=30)
#print(tf.__version__)

titleLabel = Label(root, text='VIDEO SEGMENTATION', font=('Times New Roman', 30, 'bold '),
                   fg='black', )
titleLabel.place(x=320, y=60, height=40)
...
def looping():
    ...
    entry.delete(0, tk.END) # Remove the current text from the entry widget
    ...
    if save_txt or save_img:
            ...
            op=(f"F:\VIDEO SEGMENT{save_dir}{s}")
            op=op.replace('..', '')
            op = os.path.join(op, files)
...
def upload_file():
        global filename
        f_types = [('Video Files', '*.mp4;*.avi;*.mkv;*.jpg;*.png;*.gif;*.jpeg')]       
        filename = filedialog.askopenfilename(title="Select a video file", filetypes=f_types)
...
##        e1 =tk.Label(root)
##        e1.place(x=370, y=270, width=240)
##        e1.image = img
##        e1['image']=img
        return filename
root.mainloop()
